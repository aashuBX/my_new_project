...
ENV JSON_LOG_PATH="/extra-01/logs/conversive_button_management" \
    APPLICATION_LOG_PATH="/application/logs/conversive_button_management"
...
# CMD sh -c 'gunicorn --bind 0.0.0.0:5010 --log-level=${DEBUG} \
...
CMD sh -c 'gunicorn --bind 0.0.0.0:5010 --log-level=debug run:app'
# CMD sh -c 'gunicorn --log-level debug run:app --bind 0.0.0.0:5010 --worker-class gevent --timeout 30 --worker-connections 256 --max-requests 1024'
